...
class _SessionEditPageState extends State<SessionEditPage> {
  ...
  List<String> _getLocalizedServices(LanguageProvider language) {
    return [
      ...
      language.getText('Педикюр', 'Педикюр'),
      language.getText('Наращування нігтів', 'Наращивание ногтей'),
      language.getText('Дизайн нігтів', 'Дизайн ногтей'),
      language.getText('Покриття гель-лак', 'Покрытие гель-лак'),
      language.getText('Зняття покриття', 'Снятие покрытия'),
      language.getText('Корекція', 'Коррекция'),
    ];
  }
  ...
  void initState() {
    ...
    for (String code in PhoneValidator.countryCodes.keys) {
      if (phoneValue.startsWith('$code ')) {
        ...
        phoneValue = phoneValue.substring(code.length + 1); // Прибираємо код та пробіл
        break;
      }
    }
        ...
        SnackBar(
          content: Consumer<LanguageProvider>(
            builder: (context, language, child) {
              return Text(language.getText('Завантаження майстринь...', 'Загрузка мастериц...'));
            },
          ),
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Consumer<LanguageProvider>(
            builder: (context, language, child) {
              return Text(language.getText('Оберіть майстриню', 'Выберите мастерицу'));
            },
          ),
          content: SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Consumer<LanguageProvider>(
                  builder: (context, language, child) {
                    final localizedName = master.getLocalizedName(language.currentLocale.languageCode);
                    return ListTile(
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (clientId == null) {
          ...
          throw Exception(language.getText('Не вдалося оновити дані клієнтки', 'Не удалось обновить данные клиентки'));
        }
        ...
        await _firestoreService.updateSession(widget.session.id!, updatedSession);
        ...
        try {
          ...
          final master = appState.masters.firstWhere((m) => m.id == updatedSession.masterId);
          
          // Перетворюємо дату та час в DateTime
          final sessionDateTime = DateTime.parse('${updatedSession.date} ${updatedSession.time}:00');
        ...
        try {
          ...
          if (existingClientId != null) {
            ...
            final existingClient = await _firestoreService.getClientById(existingClientId);
            if (existingClient != null) {
              ...
              await _firestoreService.updateClient(existingClientId, updatedClient);
            }
          }
        } catch (e) {
                ...
                children: [
                  ...
                  Expanded(
                    child: Consumer<LanguageProvider>(
                      builder: (context, language, child) {
                        return Text(
                          language.getText('Запис успішно оновлено!', 'Запись успешно обновлена!'),
                          style: TextStyle(
                            fontSize: 16,
                ...
                children: [
                  ...
                  Expanded(
                    child: Consumer<LanguageProvider>(
                      builder: (context, language, child) {
                        return Text(
                          language.getText('Помилка оновлення: $e', 'Ошибка обновления: $e'),
                          style: TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.w500,
                          ),
    ...
    final bool? confirm = await showDialog<bool>(
      ...
      builder: (BuildContext context) {
        return Consumer<LanguageProvider>(
          builder: (context, language, child) {
            return AlertDialog(
              title: Text(language.getText('Видалити запис?', 'Удалить запись?')),
              content: Text(language.getText('Ви впевнені, що хочете видалити цей запис? Цю дію неможливо скасувати.', 'Вы уверены, что хотите удалить эту запись? Это действие нельзя отменить.')),
              actions: [
                TextButton(
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Consumer<LanguageProvider>(
                builder: (context, language, child) {
                  return Text(language.getText('Помилка видалення: $e', 'Ошибка удаления: $e'));
                },
              ),
              backgroundColor: Colors.red[600],
            ),
    ...
    return ConnectivityWrapper(
      ...
      appBar: AppBar(
        ...
        title: Consumer<LanguageProvider>(
          builder: (context, language, child) {
            return Text(
              language.getText('Редагування запису', 'Редактирование \nзаписи'),
              style: TextStyle(
                fontSize: 24,
              ...
              child: Column(
                ...
                children: [
                                      ...
                                      Row(
                                        children: [
                                            ...
                                            child: Consumer<AppStateProvider>(
                                              builder: (context, appState, child) {
                                                final master = appState.masters.firstWhere(
                                                  ...
                                                  orElse: () => appState.masters.isNotEmpty ? appState.masters.first : Master(name: 'Невідомо', status: 'active'),
                                                );
                                                final localizedName = master.getLocalizedName(language.currentLocale.languageCode);
                                                return Text(
                                                  language.getText('Майстриня: $localizedName', 'Мастерица: $localizedName'),
                                                  style: TextStyle(
                                                    fontSize: 18,
                                                    fontWeight: FontWeight.bold,
                                          ...
                                          IconButton(
                                            ...
                                            tooltip: language.getText('Змінити майстриню', 'Изменить мастерицу'),
                  ...
                  Consumer<LanguageProvider>(
                    builder: (context, language, child) {
                      return Text(
                        language.getText('Інформація про клієнтку', 'Информация о клиентке'),
                        style: TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.bold,
                  ...
                  Consumer<LanguageProvider>(
                    builder: (context, language, child) {
                      return TextFormField(
                        ...
                        decoration: InputDecoration(
                          labelText: language.getText('Примітки про клієнтку', 'Заметки о клиентке'),
                  ...
                  Consumer<LanguageProvider>(
                    builder: (context, language, child) {
                      final localizedServices = _getLocalizedServices(language);
                      return DropdownButtonFormField<String>(
                        ...
                        items: _originalServices.asMap().entries.map((entry) {
                          ...
                          return DropdownMenuItem<String>(
                            value: originalService, // Сохраняем оригинальное значение в БД
                            child: Text(localizedService), // Показываем локализованное
                          );
                        }).toList(),
                        onChanged: (String? newValue) {
                          setState(() {
                            ...
                            return DropdownButtonFormField<int>(
                              ...
                              items: _durations.map((int duration) {
                                return DropdownMenuItem<int>(
                                  value: duration,
                                  child: Text(language.getText('$duration хв', '$duration мин')),
                                );
                              }).toList(),
                  ...
                  Consumer<LanguageProvider>(
                    builder: (context, language, child) {
                      return TextFormField(
                        ...
                        decoration: InputDecoration(
                          labelText: language.getText('Примітки (необов\'язково)', 'Заметки (необязательно)'),
